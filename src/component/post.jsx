...
function PostPage () {
  ...
  const [reviewOrDescription , setreviewOrDescription] =useState("description")
    ...
    const  [reviewRateIs , setreviewRateIs] =useState(0)
...
const HandleRatingSubmit = async(e) =>{
  ...
  for (let i = 1; i < 6; i++) {
console.log(reviewRateIs)
console.log(i)
    
    if(i < ( reviewRateIs +1)){
      console.log(i)
      reviewStarIcons.push(<i key={i} onClick={() => HandleRatingSubmit(i)} className="fa fa-star text-gold"></i>)
    }else{
      reviewStarIcons.push(<i key={i} onClick={() => HandleRatingSubmit(i)} className="fa fa-star-o text-gold"></i>)
    }
  }
                    ...
                    <div id="tab2" class="tab-content p-4">
                      ...
                      <form onSubmit={addReviewHandle} class="w-full border p-[20px]">
                        ...
                        <div class="w-[100%] sm:w-[45%]">
                          ...
                          <p class=" text-base pt-3">These are {`(${rewiesArray.length})`}</p>
...
<PopUpMsg  status={200} message={servermsg} />
